...
# --------------------------
# 修复：添加当前目录到路径，确保能找到模型文件
# --------------------------
def get_model_path(model_path='best_model.pth'):
	"""获取模型文件的正确路径"""
	# 如果是打包后的可执行文件
	...
	# 设置模型文件路径
	full_path = os.path.join(application_path, model_path)

	# 如果模型文件不存在，尝试在当前工作目录查找
...
def load_model(model_path='best_model.pth'):
	...
	try:
		...
		# 添加更详细的错误信息
		raise Exception(f"加载模型失败: {str(e)}。尝试的路径: '{full_model_path}'")
...
class ScreenRecognitionApp:
	def __init__(self, root):
		...
		# 坐标文件路径 - 使用修复的路径处理
		self.regions_file = self.get_config_path("regions.json")
